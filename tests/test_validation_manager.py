...
class TestValidateState:
    ...
    def test_validate_state_file_not_found_adds_warning(
        self, validation_manager, temp_dir
    ):
    ...
    def test_validate_state_tasks_json_missing_tasks_key_adds_error(
        self, validation_manager, temp_dir
    ):
    ...
    def test_validate_state_corruption_error_adds_error(
        self, validation_manager, temp_dir
    ):
        ...
        assert result.valid is False
        assert any(
            "corrupt" in e.lower() or "corrupted" in e.lower() for e in result.errors
        )

    def test_validate_state_generic_exception_adds_error(
        self, validation_manager, temp_dir
    ):
...
class TestRecoverFromCorruption:
    ...
    def test_recover_from_checkpoint_success(self, validation_manager):
        ...
        validation_manager._checkpoint.restore_checkpoint.assert_called_once_with(
            "latest"
        )

    def test_recover_from_checkpoint_restore_raises_tries_backup(
        self, validation_manager, temp_dir
    ):
        ...
        validation_manager._checkpoint.restore_checkpoint.side_effect = RuntimeError(
            "restore failed"
        )
    ...
    def test_recover_no_checkpoint_no_backup_returns_false(
        self, validation_manager, temp_dir
    ):
