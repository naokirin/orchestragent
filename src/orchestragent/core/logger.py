...
class AgentLogger:
    ...
    def log_error_with_traceback(
        ...
        # Log to standard logger
        self.logger.error(f"[{agent_name}] Error: {type(error).__name__}: {error}")
    ...
    def log_agent_command_output(
        self, agent_name: str, stdout: str, stderr: str, command: Optional[str] = None
    ) -> Path:
        """
    ...
    def start_agent_command_stream(
        self, agent_name: str, command: Optional[str] = None
